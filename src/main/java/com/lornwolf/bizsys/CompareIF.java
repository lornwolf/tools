...
public class CompareIF {
    ...
    private void fetchAllFiles(File dir) {
        ...
        // 新版的相应目录下的所有外部IF设计书文件（用来比较新版中是否有新增的文件）。
        File[] files221 = null;

        // 新版「040_外部IF」目录下的所有目录（用来比较新版中是否有新增的目录）。
        File[] folders221 = null;
        if ("040_外部IF".equals(dir.getName())) {
            folders221 = new File(dir.getPath().replace(OLD_VERSION, NEW_VERSION)).listFiles();
        }
        ...
        if (files.length > 0) {
            for (File file201 : files) {
                ...
                /* 临时代码 开始 */
                if (file201.getPath().indexOf("07_グループ会計") >= 0) {
                    continue;
                }
                /* 临时代码 结束 */

                if (file201.isFile() && (file201.getName().startsWith("外部I／F定義") || file201.getName().startsWith("外部Ｉ／Ｆ定義"))) {

                    System.out.println(file201.getPath().substring(input201.length() + 1));
                    String file221 = file201.getPath().replace(OLD_VERSION, NEW_VERSION);
                    ...
                    if (files221 == null) {
                        if (new File(file201.getParent().replace(OLD_VERSION, NEW_VERSION)).exists()) {
                            files221 = new File(file201.getParent().replace(OLD_VERSION, NEW_VERSION)).listFiles();
                        }
                    }
                }
            ...
            if (files221 != null) {
                ...
                for (String fileName : list221) {
                    if (!list201.contains(fileName)) {
                        ...
                        try {
                            tmpSheet = new XSSFWorkbook(new FileInputStream(dir.getPath().replace(OLD_VERSION, NEW_VERSION) + "\\" + fileName)).getSheetAt(0);
                        } catch (Exception e) {}
            ...
            if (folders221 != null) {
                for (int i = 0; i < folders221.length; i++) {
                    if (new File(folders221[i].getPath().replace(NEW_VERSION, OLD_VERSION)).exists()) {
                        continue;
                    }
    ...
    private void compare(File file201, File file221) {
        try (XSSFWorkbook workbook201 = new XSSFWorkbook(new FileInputStream(file201));
            ...
            for (int index = 0; index < workbook201.getNumberOfSheets(); index++) {
                ...
                if (sheetName201.equals("イメージ") || sheetName201.equals("Sheet1")) {
                    continue;
                }
                // 比较一下Sheet名。
    ...
    private String getUsecase(XSSFSheet sheet) {
        ...
        if (!checkUsecase(usecase)) {
            usecase = ExcelUtil.getStr(ExcelUtil.getCell(sheet, 0, 25));
        }
        if (!checkUsecase(usecase)) {
    ...
    private int getSheetCount(XSSFWorkbook workbook) {
        ...
        for (int i = 0; i < workbook.getNumberOfSheets(); i++) {
            ...
            if (!sheetName.equals("イメージ") && !sheetName.equals("Sheet1")) {
                count += 1;
            }
        }
        return count;
